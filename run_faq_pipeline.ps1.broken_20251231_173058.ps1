...
# WARMUP_GENERATE_CACHE (biz9 parking variants etc)
$warmQs = @(
  "Paid parking",
...
foreach ($q in $warmQs) {
  try {
    & curl.exe -sS --http1.1 -X POST "https://api.motionmadebne.com.au/api/v2/generate-quote-reply" 
      -H "Content-Type: application/json" 
      --data-binary ("{""tenantId"":"""",""customerMessage"":""" + ($q -replace '"','\"') + """}")
  } catch {}
}

# If suite fails, retry once before rollback (handles propagation / cache)
$out = powershell -ExecutionPolicy Bypass -NoProfile -File .\run_suite.ps1 -TenantId $TenantId 2>&1 | Out-String
if ($out -match "FAIL:") {

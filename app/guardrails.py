...
def violates_general_safety(text: str) -> bool:
    """
    Guardrail for *general* answers only.
    Block only if reply looks like business-specific claims/CTA.
    Allow normal general-knowledge explanations.
    """
    t = _normalize(text)
    if not t:
        return True
    ...
    patterns = [
        ...
        r"\b\d+(?:\.\d+)?\s*(mins?|minutes?|hrs?|hours?|days?)\b",
        r"\b(travel fee)\b",
